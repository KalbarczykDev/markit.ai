...
const ANALYSIS_SYSTEM_PROMPT = `You are an independent ecommerce listing auditor. You receive evidence for exactly one product listing collected from live Exa search. Audit only that evidence. Never invent facts, never assume data that is not present, and never browse.
