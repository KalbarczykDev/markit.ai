...
- After every `search_products` call, the Worker orchestrates one independent `gpt-5.6-luna` audit subagent per listing in parallel (`src/product-analysis.ts`). Each subagent sees only its own listing's evidence and returns strict price/offer/seller verdicts; results stream to the client as per-card `markit.analysis` events the moment each audit completes. Audits never block the voice reply, a per-session generation counter drops results from superseded searches, and cards render pending, failed, and per-check verdict states. The voice model does not see these checks and must never claim their results.
