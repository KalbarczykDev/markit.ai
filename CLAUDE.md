...
- Independent listing audits run after every search: the Worker fans out one `gpt-5.6-luna` subagent per listing in parallel (`src/product-analysis.ts`) and streams per-card `markit.analysis` events as each completes. Audits never block the voice reply, stale generations are dropped, and cards show pending/failed/verdict check states the voice model must never claim.
